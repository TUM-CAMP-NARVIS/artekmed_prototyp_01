...
# BPF program
bpf_text_shared = "%s/bpf_text_dataflow_shared.c" % os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))
 ...
 */
BPF_PERF_OUTPUT(eventqueue_event);
 ...
 */
int trace_eventqueue_dispatch_end(struct pt_regs* ctx)
{
    ...
    struct start_data_t* start_data = start_hash.lookup(&event);
    if (0 == start_data) {
        return 0;
    }
    ...
    bpf_usdt_readarg(2, ctx, &end_data.timestamp);
    ...
    eventqueue_event.perf_submit(ctx, &end_data, sizeof(end_data));
    return 0;
}
"""
...
if this_filter:
    bpf_text = bpf_text.replace("FILTER", "if (!filter(ev.component_name)) { return 0; }")
else:
...
usdt_ctx.enable_probe(probe="eventqueue_dispatch_begin", fn_name="trace_eventqueue_dispatch_begin")
usdt_ctx.enable_probe(probe="eventqueue_dispatch_end", fn_name="trace_eventqueue_dispatch_end")
...
def print_event(cpu, data, size):
    ...
    print("%-18.9f %-10d %-32s %-32s %16d %16d %16d" % (time_s, event.timestamp, event.component_name, event.port_name, event.start, event.end, latency))
...
bpf_ctx["eventqueue_event"].open_perf_buffer(print_event)
while 1:
